...
/* PointerEvent */
let pointerId: pointerId = event->pointerId
let width: int = event->width
let height: int = event->height
let pressure: float = event->pressure
let tiltX: int = event->tiltX
let tiltY: int = event->tiltY
let pointerType: pointerType = event->pointerType
let isPrimary: bool = event->isPrimary
