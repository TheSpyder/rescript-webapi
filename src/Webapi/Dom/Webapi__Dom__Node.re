module Impl = (T: {type t;}) => {
  ...
  */
  ...
  [@send]  external getRootNodeComposed: (T.t, [@as {json|{ "composed": true }|json}] _) => Dom.node = "getRootNode";
  [@send] external hasChildNodes: T.t => bool = "";
  [@send]  external insertBefore: (T.t, Dom.node_like('a), Dom.node_like('b)) => Dom.node_like('a) = "";
  ...
  [@send] [@return nullable] external lookupDefaultNamespaceURI: T.t => ([@as {json|null|json}] _) => option(string) = "lookupNamespaceURI";
  ...
  /** @since 0.19.0 */
  [@send]  external replaceChild: (T.t, Dom.node_like('a), Dom.node_like('b)) => Dom.node_like('b) = "";
};
