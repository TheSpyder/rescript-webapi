'use strict';

var Webapi__Dom__Types$Webapi = require("../../../src/Dom/Webapi__Dom__Types.js");
var Webapi__Dom__Document$Webapi = require("../../../src/Dom/Webapi__Dom__Document.js");
var Webapi__Dom__NodeFilter$Webapi = require("../../../src/Dom/Webapi__Dom__NodeFilter.js");

var el = document.createElement("strong");

Webapi__Dom__Document$Webapi.compatMode(document);

document.selectedStyleSheetSet = "muh-stylesheet";

Webapi__Dom__Document$Webapi.visibilityState(document);

document.adoptNode(el);

document.createAttribute("data-foo");

document.createAttributeNS("http://...", "foo");

document.createComment("witty comment");

document.createDocumentFragment();

document.createElement("div");

document.createElement("div", {});

document.createElementNS("http://...", "foo");

document.createElementNS("http://...", "div", {});

document.createEvent("MyCustomEvent");

document.createNodeIterator(el);

document.createNodeIterator(el, Webapi__Dom__Types$Webapi.WhatToShow._All);

document.createNodeIterator(el, Webapi__Dom__Types$Webapi.WhatToShow.many({
          hd: Webapi__Dom__Types$Webapi.WhatToShow._Element,
          tl: {
            hd: Webapi__Dom__Types$Webapi.WhatToShow._Attribute,
            tl: /* [] */0
          }
        }), Webapi__Dom__NodeFilter$Webapi.make(function (param) {
          return 0;
        }));

document.createRange();

document.createTextNode("Very reasonable!");

document.createTreeWalker(el);

document.createTreeWalker(el, Webapi__Dom__Types$Webapi.WhatToShow._All);

document.createTreeWalker(el, Webapi__Dom__Types$Webapi.WhatToShow.many({
          hd: Webapi__Dom__Types$Webapi.WhatToShow._Element,
          tl: {
            hd: Webapi__Dom__Types$Webapi.WhatToShow._Attribute,
            tl: /* [] */0
          }
        }), Webapi__Dom__NodeFilter$Webapi.make(function (param) {
          return 0;
        }));

document.elementFromPoint(0, 0);

document.elementsFromPoint(0, 0);

document.enableStyleSheetsForSet("my-stylesheet-set");

document.exitPointerLock();

document.getAnimations();

document.getElementsByClassName("lstlisting");

document.getElementsByTagName("code");

document.getElementsByTagNameNS("http://...", "foo");

document.importNode(el);

document.importNode(el, true);

document.getElementById("root");

document.querySelector(".lstlisting");

document.querySelectorAll(".lstlisting");

exports.el = el;
/* el Not a pure module */
